...
public class GunSmithTableIngredientSerializer implements JsonDeserializer<GunSmithTableIngredient> {
    ...
    public GunSmithTableIngredient deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
        if (json.isJsonObject()) {
            ...
            // Migrado para NeoForge 1.21.1: Ingredient.fromJson() -> Ingredient.CODEC.parse()
            Ingredient ingredient = Ingredient.CODEC.parse(JsonOps.INSTANCE, jsonObject.get("item"))
                .getOrThrow(error -> new JsonSyntaxException("Failed to parse ingredient: " + error));
            int count = 1;
